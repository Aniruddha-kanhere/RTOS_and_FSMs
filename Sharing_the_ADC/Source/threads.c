...
void Thread_ADC_server(void *arg);

osThreadId_t t_Read_TS, t_Read_Accelerometer, t_Sound_Manager, t_US, t_Refill_Sound_Buffer, t_BUS, t_Server;
osMessageQueueId_t ADC_server_message_q;
volatile char queued_adc=0;
...
void Create_OS_Objects(void) {
	...
	ADC_server_message_q = osMessageQueueNew(5,sizeof(Queue),NULL);
...
/*void Thread_ADC_server(void *arg)
{
	...
	while(1)
	{
		//timer overflow leading to ADC conversion completing sets this flag in ADC_IRQHandler
		DEBUG_TOGGLE(DBG_3);
		
		osThreadFlagsWait(1,osFlagsWaitAll,osWaitForever);
				
		if(osMessageQueueGetCount(ADC_server_message_q))
		{
			while(osMessageQueueGetCount(ADC_server_message_q))   //While there are pending requests
			{			
				queued_adc = 1;
				//Get the message from queue.
				osMessageQueueGet(ADC_server_message_q,(void *)&q,NULL,osWaitForever);
				//Initialise the conversion
				ADC0->SC1[0] &= ~ADC_SC1_ADCH_MASK;
				ADC0->SC1[0] |= q.ChannelNum;
				//Wait for the conversion
				osThreadFlagsWait(1,osFlagsWaitAll,osWaitForever);		
				//store the result in the pointer provided
				*q.ResultPtr = ADC0->R[0];
				//Allow the thread to run
				osThreadFlagsSet(q.TID,q.FlagNum);			
			}
			ADC0->SC1[0] &= ~ADC_SC1_ADCH_MASK;
...
void Thread_Read_TS(void * arg) {
	...
	while (1) {
		...
		if (LCD_TS_Read(&p)) 
		{
			if (p.Y > ROW_TO_Y(LCD_MAX_ROWS-3) + 10) 
			{
				// Update g_peak_set_current based on X position of touch
				g_peak_set_current = (int)p.X;
			}
			else 
			{
				// Else draw on screen
 ...
 void Thread_Update_Screen(void * arg) {
	...
	while (1) {
		...
		sprintf(buffer, "I Peak: %d   ",g_peak_set_current);
		osMutexAcquire(LCD_mutex, osWaitForever);
		LCD_Text_PrintStr_RC(2, 0, buffer);
		osMutexRelease(LCD_mutex);
				
		if ((roll < -2.0) || (roll > 2.0) || (!start_flag)) {
			start_flag = 1;
			p1.X = paddle_pos;
			p1.Y = PADDLE_Y_POS;
