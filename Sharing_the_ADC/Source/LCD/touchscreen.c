...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	if (b>0) {
		...
		q.FlagNum = 1;		
		q.ResultPtr = &x;
		q.ChannelNum = LCD_TS_YU_CHANNEL;
		q.valid = 1;
				
		osMessageQueuePut(my_os_q, &q, 1, osWaitForever);
				
		osThreadFlagsWait(q.FlagNum,osFlagsWaitAll,osWaitForever);
		...
		q.FlagNum = 1;		
		q.ResultPtr = &y;
		q.ChannelNum = LCD_TS_XL_CHANNEL;
		q.valid=1;
