...
void ADC0_IRQHandler() {
	...
	switch(prev)
	{
		case Priority:
			Control_HBLED();		
		  if(osMessageQueueGetCount(my_os_q))
			{
				osMessageQueueGet(my_os_q, &my_q, &pri, 0);
				
				prev = Queued;
				ADC0->SC2 &= ~ADC_SC2_ADTRG_MASK;
				ADC0->SC2 |= ADC_SC2_ADTRG(0);
				ADC0->SC1[0] &= ~ADC_SC1_ADCH_MASK;
				ADC0->SC1[0] |= my_q.ChannelNum;
			}
			break;
		case Queued:
			*my_q.ResultPtr = ADC0->R[0];
		  osThreadFlagsSet(my_q.TID,my_q.FlagNum);
